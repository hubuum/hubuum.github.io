...
CONCEPTS = [
    ...
    dict(slug="orbit", number="02", name="Orbit", mood="Luminous · precise · centred",
         ...
         theme="Deep ember, luminous gold, orange, and coral orbital geometry."),
...
def data_node(x, y, structured=True, size=1, ink="#4b7f80", paper="#f5f8f2"):
...
def orbit():
    ...
    <radialGradient id="halo"><stop stop-color="#ff9d48" stop-opacity=".3"/><stop offset=".42" stop-color="#d44536" stop-opacity=".16"/><stop offset="1" stop-color="#511f25" stop-opacity="0"/></radialGradient>
    <radialGradient id="pearl" cx=".35" cy=".2"><stop stop-color="#fff3bd"/><stop offset=".52" stop-color="#ffd17a"/><stop offset="1" stop-color="#ee7650"/></radialGradient>
    <linearGradient id="arc"><stop stop-color="#d45c4a" stop-opacity=".12"/><stop offset=".35" stop-color="#ffb75b" stop-opacity=".65"/><stop offset=".65" stop-color="#ffda83" stop-opacity=".75"/><stop offset="1" stop-color="#e16750" stop-opacity=".18"/></linearGradient>
    ...
    for index, (x, y, label) in enumerate([(247, 380, "CONFIGURATION"), (447, 170, "SERVICES"), (958, 191, "YOUR OWN DATA"), (1128, 421, "RESOURCES"), (551, 573, "CONTEXT")]):
        parts.append(f'<path d="M700 380 Q{(700+x)/2} {y} {x} {y}" fill="none" stroke="#e39762" stroke-width="1" opacity=".4"/>' + data_node(x, y, structured=index % 2 == 1, ink="#ffc37d", paper="#2f1c20") + f'<text x="{x}" y="{y+46}" text-anchor="middle" font-size="11" letter-spacing="2.3" fill="#ddb798" font-family="system-ui,sans-serif">{label}</text>')
    for i in range(42):
        angle = i * 2.39996
        radius = 160 + (i * 31) % 400
        x, y = 700 + math.cos(angle) * radius, 380 + math.sin(angle) * radius * .66
        parts.append(f'<circle cx="{x:.2f}" cy="{y:.2f}" r="1.4" fill="#f4b389" opacity=".5"/>')
    parts.extend(['<circle cx="700" cy="380" r="101" fill="none" stroke="#ffbe73" stroke-opacity=".13"/><circle cx="700" cy="380" r="80" fill="none" stroke="#ffbe73" stroke-opacity=".25"/><circle cx="700" cy="380" r="59" fill="url(#pearl)"/>', mark(700, 387, 1.05, "#642b1c"), '<text x="700" y="506" text-anchor="middle" font-family="system-ui,sans-serif" font-size="11" letter-spacing="4" fill="#f2c68b">HUBUUM</text>', '</svg>'])
    return "\n".join(parts)
...
def story_art(warm=False):
    palette = {
        "wash": "#c97246" if warm else "#61988f",
        "line": "#de9c63" if warm else "#75a59d",
        "label": "#e6c4a7" if warm else "#bdd4ce",
        "panel": "#332025" if warm else "#102f36",
        "border": "#b67b58" if warm else "#779c95",
        "kind": "#e4be91" if warm else "#b6cfc9",
        "title": "#fff1d9" if warm else "#f1f5ee",
        "schema": "#f2d19f" if warm else "#c2d9cd",
        "authority": "#cfac92" if warm else "#9fbab4",
    }
    return svg_start("Example CMDB: schema-bound Atlas and web-01 connect to schema-free research notes and Oslo; authority lives in Hubuum or upstream independently of schema choice", "0 0 800 580") + string.Template('''
    <defs><radialGradient id="wash"><stop stop-color="$wash" stop-opacity=".2"/><stop offset="1" stop-color="$wash" stop-opacity="0"/></radialGradient></defs>
    <circle cx="400" cy="280" r="270" fill="url(#wash)"/>
    <g fill="none" stroke="$line" stroke-width="1.2"><path d="M350 172H450 M235 250C235 300 210 300 210 350 M570 250C570 300 595 300 595 350"/></g>
    <g font-family="system-ui,sans-serif" text-anchor="middle">
    <g fill="$label" font-size="13"><text x="400" y="154">runs on</text><text x="642" y="307">located in</text><text x="154" y="307">described by</text></g>
    <g fill="$panel" stroke="$border"><rect x="105" y="90" width="245" height="160" rx="5"/><rect x="450" y="90" width="245" height="160" rx="5"/></g>
    <g fill="$panel" stroke="$border" stroke-dasharray="5 6"><rect x="85" y="350" width="250" height="160" rx="16"/><rect x="460" y="350" width="250" height="160" rx="16"/></g>
    <g fill="$kind" font-size="11" letter-spacing="2"><text x="228" y="121">SERVICE</text><text x="573" y="121">SERVER</text><text x="210" y="382">CONTEXT</text><text x="585" y="382">LOCATION</text></g>
    <g fill="$title" font-size="27"><text x="228" y="165">Atlas</text><text x="573" y="165">web-01</text><text x="210" y="425" font-size="23">Research notes</text><text x="585" y="425">Oslo</text></g>
    <g fill="$schema" font-size="13"><text x="228" y="204">Schema-bound</text><text x="573" y="204">Schema-bound</text><text x="210" y="465">Schema-free</text><text x="585" y="465">Schema-free</text></g>
    <g fill="$authority" font-size="11"><text x="228" y="229">Authority: Hubuum</text><text x="573" y="229">Authority: upstream</text><text x="210" y="490">Authority: Hubuum</text><text x="585" y="490">Authority: upstream</text></g></g>
    </svg>''').substitute(palette)
...
def render():
    ...
    (assets / "relationships-orbit.svg").write_text(story_art(warm=True) + "\n")
    favicon = (assets / "favicon.svg").read_text()
    (assets / "favicon-orbit.svg").write_text(
        favicon.replace("#174f57", "#211419").replace("#e5f2e9", "#ffd079")
    )
    ...
    for concept in CONCEPTS:
        ...
        content = template.substitute(**concept, headline=HEADLINE, description=DESCRIPTION,
                                      ...
                                      theme_color="#211419" if warm else "#174f57",
                                      favicon="favicon-orbit" if warm else "favicon",
                                      relationships_art="relationships-orbit" if warm else "relationships")
